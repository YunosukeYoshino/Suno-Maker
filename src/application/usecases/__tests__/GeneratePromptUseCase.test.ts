...
describe('GeneratePromptUseCase', () => {
  ...
  describe('入力バリデーション', () => {
    it('ジャンルが空の場合エラーをスローする', async () => {
      ...
      await expect(useCase.execute(input)).rejects.toThrow('ジャンルを選択してください');
    });
    ...
    it('ジャンルが5つを超える場合エラーをスローする', async () => {
      ...
      await expect(useCase.execute(input)).rejects.toThrow('ジャンルは最大5つまで選択できます');
    });
  });
  ...
  describe('最適化処理', () => {
    it('最適化サービスがない場合でも基本プロンプトを返す', async () => {
      const useCaseWithoutOptimization = new GeneratePromptUseCase(mockRepository);
